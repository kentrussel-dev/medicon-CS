  ...
  <Modal :is-open="isOpen" title="Book Doctor Appointment" subtitle="Select your preferred specialist and time slot" size="lg" @close="$emit('close')">
    ...
    <template #footer>
      ...
      >
        ...
        <span v-else>Proceed to Checkout Page (₱{{ selectedFeePesos }})</span>
      </button>
    </template>
  </Modal>
</template>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const newApptId = res?.id || Date.now()
    emit('booked')
    emit('close')

    // Navigate directly to the dedicated Checkout page
    router.push({
      name: 'patient-checkout',
      params: { appointmentId: newApptId },
    })
  } catch (err) {
    // Handled in store
  } finally {
