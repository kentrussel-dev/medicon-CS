<template>
  <Modal :is-open="isOpen" title="Book Doctor Appointment" subtitle="Select your preferred specialist and time slot" size="lg" @close="$emit('close')">
    <form @submit.prevent="handleSubmit" class="space-y-4">
      <!-- Doctor Selection -->
      <div>
        <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">Select Specialist</label>
        <select
        ...
        >
          ...
          <option v-for="doc in doctors" :key="doc.id" :value="doc.id">
            {{ doc.name }} &bull; {{ doc.specialty }} (${{ doc.consultation_fee }})
          </option>
        </select>
      </div>
      ...
      <!-- Appointment Type -->
      <div>
        <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">Consultation Format</label>
        <div class="grid grid-cols-2 gap-3">
          <label
            class="flex items-center justify-center p-3 rounded-xl border cursor-pointer text-sm font-semibold transition-all"
            :class="form.type === 'TELEHEALTH' ? 'border-brand-500 bg-brand-50 text-brand-800' : 'border-slate-200 text-slate-600 hover:bg-slate-50'"
          >
          ...
          <label
            class="flex items-center justify-center p-3 rounded-xl border cursor-pointer text-sm font-semibold transition-all"
            :class="form.type === 'IN_PERSON' ? 'border-brand-500 bg-brand-50 text-brand-800' : 'border-slate-200 text-slate-600 hover:bg-slate-50'"
          >
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">Appointment Date</label>
          <input
            type="date"
            v-model="form.date"
            :min="minDate"
        ...
        <div>
          <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">Time Slot (30 mins)</label>
          <select
            v-model="form.time"
            required
            class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500 bg-white"
      ...
      <!-- Reason for Visit -->
      <div>
        <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">Reason for Consultation</label>
        <textarea
          v-model="form.reason"
          required
          rows="2"
    ...
    <template #footer>
      ...
      <button
        ...
        :disabled="submitting"
        @click="handleSubmit"
        class="px-5 py-2 rounded-xl text-sm font-semibold bg-brand-600 hover:bg-brand-700 text-white transition-colors shadow-sm disabled:opacity-50 flex items-center"
      >
        <span v-if="submitting">Booking...</span>
        <span v-else>Confirm & Book Visit</span>
      </button>
    </template>
  </Modal>
</template>
